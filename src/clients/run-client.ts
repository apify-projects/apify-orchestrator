...
export class ExtRunClient extends RunClient implements ExtendedRunClient {
    ...
    override async get(options?: RunGetOptions): Promise<ActorRun | undefined> {
        ...
        if (run) {
            await this.context.runTracker.updateRun(this.runName, run);
        } else {
            await this.context.runTracker.declareLostRun(this.runName, 'Actor client could not retrieve the Run');
        }
        return run;
    }
