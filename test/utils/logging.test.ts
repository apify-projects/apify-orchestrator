    ...
    describe('Logger', () => {
        describe('prefixed', () => {
            it('creates a child logger with the correct prefix', () => {
                ...
                generateLogger({ enableLogs: true, hideSensitiveInformation: false }).prefixed('test');
        ...
        describe('debug', () => {
            it('logs debug messages when enabled', () => {
                ...
                const logger = generateLogger({ enableLogs: true, hideSensitiveInformation: false });
                logger.debug('test');
            ...
            it('hides sensitive data when required', () => {
                const debugSpy = vi.spyOn(log, 'debug');
                const logger = generateLogger({ enableLogs: true, hideSensitiveInformation: true });
                logger.debug('test', undefined, { secret: 'data' });
                expect(debugSpy).toHaveBeenLastCalledWith('test', undefined);
            });
